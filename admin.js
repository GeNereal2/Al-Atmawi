...
function renderCompaniesList() {
  ...
  adminCompaniesList.innerHTML = companies.map(company => {
    const companyProductsCount = getProducts().filter(product => String(product.companyId) === String(company.id)).length;
...
function deleteCompany(companyId) {
  ...
  const updatedProducts = getProducts().filter(product => String(product.companyId) !== String(companyId));
